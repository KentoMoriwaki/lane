...
import { fetchTasksByIds } from "@/app/react-query-rsc/api/endpoints";
import {
  ...
  tasksQueryOptions,
  teamsQueryOptions,
} from "@/app/react-query-rsc/api/query-options";
import {
...
import { Workspace } from "@/app/react-query-rsc/workspace/workspace";
import { WorkspaceLoadingShell } from "@/app/react-query-rsc/workspace/workspace-loading-shell";
import { WorkspaceProvider } from "@/app/react-query-rsc/workspace/workspace-provider";
import {
...
async function WorkspaceHydration({ searchParams }: PageProps) {
  ...
  if (requested.teamId && !teams.some((team) => team.id === requested.teamId)) {
    ...
    redirect(search ? `/react-query-rsc?${search}` : "/react-query-rsc");
  }
