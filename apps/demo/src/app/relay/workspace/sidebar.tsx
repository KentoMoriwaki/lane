...
import type { RelayShellQuery$data } from "@/app/relay/__generated__/RelayShellQuery.graphql";
import type { sidebarUser_viewer$key } from "@/app/relay/__generated__/sidebarUser_viewer.graphql";
import type { sidebarNav_query$key } from "@/app/relay/__generated__/sidebarNav_query.graphql";
import { TeamSwitcher } from "./team-switcher";

const sidebarUserFragment = graphql`
...
// The slow derived reads (projects/labels) and the counters are `@defer`-ed in
// the shell query, so this fragment's data streams in after the chrome paints.
const sidebarNavFragment = graphql`
...
export function Sidebar({
  ...
  query: RelayShellQuery$data;
  filters: TaskFilters;
  onViewChange: (view: Partial<TaskFilters>) => void;
}) {
  const user = useFragment<sidebarUser_viewer$key>(
    sidebarUserFragment,
    query.viewer,
  );
  ...
  return (
    <aside className="hidden w-60 shrink-0 flex-col border-r border-border bg-sidebar md:flex">
      ...
      <div className="px-3 pb-2">
        <TeamSwitcher query={query} />
      </div>

      <nav className="scrollbar-calm flex-1 space-y-5 overflow-y-auto px-2 py-2">
        <React.Suspense fallback={<SidebarNavSkeleton />}>
          <SidebarNav
            query={query}
            active={activeKey(filters)}
...
function SidebarNav({
  ...
  query: sidebarNav_query$key;
  active: string;
  onViewChange: (view: Partial<TaskFilters>) => void;
}) {
  const { projects, labels, insights } = useFragment(sidebarNavFragment, query);
