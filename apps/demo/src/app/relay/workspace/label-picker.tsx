...
import { useCreateLabel } from "@/app/relay/api/mutations";
import {
  Command,
  CommandEmpty,
  CommandGroup,
...
import type { labelPickerQuery } from "@/app/relay/__generated__/labelPickerQuery.graphql";
...
export function LabelPicker({
  ...
  selectedIds: string[];
  addAction: (label: PickerLabel) => void;
...
function LabelOptions({
  ...
  const { labels } = useLazyLoadQuery<labelPickerQuery>(
    labelsQuery,
    {},
  ...
  function toggle(label: PickerLabel) {
      ...
      <CommandList>
        ...
        <CommandGroup>
          {labels.map((label) => (
            ...
            >
              <span
                className={cn("size-2 rounded-full", accent(label.color).dot)}
              />
        ...
        {canCreate ? (
          ...
          >
            {isCreating ? <InlineSpinner /> : <Plus className="size-4" />}
            Create “{trimmed}”
          </button>
        ) : null}
      </CommandList>
