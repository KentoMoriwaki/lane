...
describe("focus refetch", () => {
  it("invalidates each focused stale entry once using the most aggressive staleTime", async () => {
    ...
    expect(observed).toHaveLength(2);
    expect(observed[1]).toBe(observed[0]);
    await expect(observed[0]).resolves.toEqual({ data: "reloaded" });
    expect(loader).toHaveBeenCalledTimes(1);
    await expect(readOrCreate(lane, ["tasks"], loader)).resolves.toEqual({ data: "reloaded" });
  });
...
describe("reconnect refetch", () => {
  it("invalidates stale entries for reconnect subscribers", async () => {
    ...
    ).resolves.toEqual({ data: "reconnected" });
  });
