...
export function useInfiniteLane<P, C>(
  ...
  const {
    ...
    // Pagination fields ride along inert; `useLane` ignores unknown options.
    useLane<InfiniteLaneValue<P, C>>({
      ...
      loader: async ({ current, meta, signal }) => {
        ...
        const pages: P[] = [];
        const params: C[] = [];
        // Page 1 re-fetches from its original cursor: a changed
        // `initialCursor` cannot silently re-anchor an existing list.
