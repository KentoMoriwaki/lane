...
import { useInsights } from "@/app/lane-spa/api/hooks";
...
function InsightCardButton({
  ...
  return (
    <Link
      ...
      onClick={onSelect}
    >
      <InsightCardLinkContent card={card} isActive={isActive} />
    </Link>
  );
}

function InsightCardLinkContent({
  card,
  isActive,
}: {
  card: InsightCard;
  ...
  return <InsightCardContent card={card} isActive={isActive || pending} />;
}
