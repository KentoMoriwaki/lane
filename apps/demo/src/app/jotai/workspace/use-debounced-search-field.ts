 ...
 *
 * `filtersAtom` stays the source of truth for the committed query; the
 * optimistic overlay is only what the user sees while the debounce and the atom
 * write are still in flight. Two details are load-bearing:
 *
 ...
 * - The round trip through the atom must be lossless (`q` is stored as typed;
 *   the fetch layer trims instead). If the committed value differed from what
 *   was typed, the controlled input would be rewritten when the transition
 *   settles — which is what destroys an in-flight IME composition.
 *
...
export function useDebouncedSearchField(
  ...
  // An in-flight navigation is never cancelled — a keystroke that has already
  // been committed just loses to whatever is written after it. This only drops
  // a commit that was still waiting out the debounce when the field went away,
  // so an unmounted search box cannot write to the URL.
  React.useEffect(
    () => () => {
