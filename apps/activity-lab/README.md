...
| `/owner-ask` | **The owner-ask, in three numbers at once**: painted frames × `refresh()` calls × RSC renders of the owning route. One lane, three published keys, a publishing route `A` and a route `B` that publishes nothing, so standing on B makes every reader of those keys a hidden `<Activity>`. The same ops panel is on both routes — which one you press is the whole experiment. Driven by `scripts/owner-ask.mjs` (six sequences: hidden `set`, visible `invalidate`, hidden `invalidate`, a `refresh` a navigation was supposed to discard, a three-key burst, hidden `update`). | An ask fired for a key nobody read, or fired more than once for keys invalidated in one run. A fallback frame on a write that carried its value (`set`), or the loss of the ~300ms one an `update` costs — either means the promise protocol changed under it. A hidden `invalidate` that asks before the reveal, or a reveal that paints the invalidated value. |
